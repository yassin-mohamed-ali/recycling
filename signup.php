...
if(isset($_POST['signup'])) {
    ...
    $confirm_password = $_POST['confirmPassword']; // added confirm password field
    $email = $_POST['email'];
    ...
    if ($age < 18) {
        echo "<script>alert('You must be 18 years or older to sign up.');</script>";
        exit; // stop execution
    }
    ...
    if ($result->num_rows > 0) {
        ...
        // Validate phone number (assuming a simple format like xxx-xxx-xxxx)
        if (!preg_match("/^\d{11}$/", $phone_number)) {
            echo "<script>alert('Invalid phone number format. Please use 11 digits only.');</script>";
            exit; // stop execution
        }
        ...
        $sql = "INSERT INTO users (user_first_name, user_last_name, user_username, user_password, user_email, user_phone_number, user_city, user_age, user_creation_date) 
                VALUES ('$first_name', '$last_name', '$username', '$hashed_password', '$email', '$phone_number', '$city', '$age', NOW())";
        if ($conn->query($sql) === TRUE) {
            header("Location: index.php");
            exit;
        } else {
            echo "<script>alert('Unable to signup. Please try again later.')</script>"; // $sql . "<br>" . $conn->error;
        }
    }
}
$conn->close();
...
<body>
  ...
  <div class="signup-container">
    ...
    <div class="form-group">
        <img src="images\blank-profile-picture-973460_960_720.webp" id="picture-enter" alt="Profile Picture" class="img img-fluid">
        <label for="profilepic">Profile Picture:</label>
        <input type="file" accept="image/*" name="profile picture" id="profileinput">
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        <div class="terms-checkbox">
            <label for="termsAndConditions" id="termsandconditionslabel">I agree to the <a href="terms_and_conditions.php" target="_blank">Terms and Conditions</a></label>
